...
import { calculateContrast, getComplementaryColor, getTriadicColors } from './color-utils';
...
export interface PaletteAnalysis {
  ...
  harmony: 'monochromatic' | 'analogous' | 'complementary' | 'triadic' | 'tetradic' | 'split-complementary' | 'custom';
...
export function getWCAGRating(ratio: number): WCAGRating {
  if (ratio >= 7) return { grade: 'AAA', level: 'Enhanced', class: 'badge-success', desc: 'Perfect for all text sizes' };
  if (ratio >= 4.5) return { grade: 'AA', level: 'Standard', class: 'badge-info', desc: 'Good for normal text' };
  if (ratio >= 3) return { grade: 'A', level: 'Large Text', class: 'badge-warning', desc: 'Only for large text (18pt+)' };
  if (ratio >= 2) return { grade: 'B', level: 'Poor', class: 'badge-warning', desc: 'Below accessibility standards' };
  if (ratio >= 1.5) return { grade: 'C', level: 'Very Poor', class: 'badge-error', desc: 'Very poor contrast' };
  return { grade: 'D', level: 'Fail', class: 'badge-error', desc: 'Completely inaccessible' };
}
 ...
 */
export function generateMonochromaticPalette(baseColor: string, count: number = 5): PaletteColor[] {
  ...
  for (let i = 0; i < count; i++) {
    const lightness = Math.max(0.1, Math.min(0.9, 0.2 + (i * 0.6) / (count - 1)));
    const color = {
      ...hsl,
      l: lightness,
    };
 ...
 */
export function generateAnalogousPalette(baseColor: string, count: number = 5): PaletteColor[] {
  const parsed = parse(baseColor);
  if (!parsed) return [];
 ...
 */
export function generateComplementaryPalette(baseColor: string): PaletteColor[] {
 ...
 */
export function generateSplitComplementaryPalette(baseColor: string): PaletteColor[] {
  const parsed = parse(baseColor);
  if (!parsed) return [];
...
export function analyzePalette(colors: PaletteColor[]): PaletteAnalysis {
  ...
  const diversity = distanceCount > 0 ? Math.min(1, totalDistance / distanceCount) : 0;
...
function determineHarmony(colors: PaletteColor[]): PaletteAnalysis['harmony'] {
  ...
  const isMonochromatic = hues.every(hue => 
    Math.abs(((hue - firstHue + 540) % 360) - 180) <= 10
  );
 ...
 */
export function improvePaletteAccessibility(colors: PaletteColor[]): PaletteColor[] {
      ...
      if (!contrast || contrast.ratio < 4.5) {
        ...
        if (parsed1 && parsed2) {
          ...
          if ((hsl1.l || 0.5) > (hsl2.l || 0.5)) {
            ...
            improved[i] = {
              ...color1,
              hex: formatHex({ ...hsl1, l: Math.min(0.95, (hsl1.l || 0.5) + 0.4) })
            };
            // Color2 is darker, make it much darker
            improved[j] = {
              ...color2,
              hex: formatHex({ ...hsl2, l: Math.max(0.05, (hsl2.l || 0.5) - 0.4) })
            };
          } else {
            // Color2 is lighter, make it much lighter
            improved[j] = {
              ...color2,
              hex: formatHex({ ...hsl2, l: Math.min(0.95, (hsl2.l || 0.5) + 0.4) })
            };
            // Color1 is darker, make it much darker
            improved[i] = {
              ...color1,
              hex: formatHex({ ...hsl1, l: Math.max(0.05, (hsl1.l || 0.5) - 0.4) })
            };
          }
        }
      }
  ...
  if (improved.length >= 2) {
    ...
    if (firstParsed) {
      ...
      improved[0] = {
        ...improved[0],
        hex: formatHex({ ...firstHsl, l: 0.9, s: Math.min(0.8, (firstHsl.s || 0.5) + 0.2) })
      };
    }
    ...
    if (lastParsed) {
      ...
      improved[lastIndex] = {
        ...improved[lastIndex],
        hex: formatHex({ ...lastHsl, l: 0.15, s: Math.min(0.9, (lastHsl.s || 0.5) + 0.3) })
      };
    }
  }
 ...
 */
export function exportPalette(palette: ColorPalette, format: 'css' | 'json' | 'ase' | 'gpl' | 'tailwind' | 'svg' | 'png'): string {
  switch (format) {
    case 'css':
...
function exportToTailwind(palette: ColorPalette): string {
  ...
  palette.colors.forEach((color, index) => {
    const name = color.name ? 
      color.name.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '') :
      `color-${index + 1}`;
...
function exportToPng(palette: ColorPalette): string {
  ...
  palette.colors.forEach((color, index) => {
    ...
    ctx.fillText(color.name || '', (index + 0.5) * swatchSize, swatchSize/2 - 10);
    ctx.font = '10px Arial';
 ...
 */
export async function extractColorsFromImage(imageSource: File | string, maxColors: number = 8): Promise<PaletteColor[]> {
 ...
 */
function extractDominantColors(imageData: ImageData, maxColors: number): PaletteColor[] {
...
export function generateColorName(hex: string): string {
  ...
  const hueNames: { [key: string]: string } = {
    0: 'Red', 15: 'Crimson', 30: 'Orange', 45: 'Amber', 60: 'Yellow',
    75: 'Lime', 90: 'Chartreuse', 120: 'Green', 150: 'Spring Green',
    180: 'Cyan', 200: 'Sky Blue', 240: 'Blue', 270: 'Violet',
    300: 'Magenta', 315: 'Rose', 330: 'Pink'
  };
  ...
  if (s > 10) { // Only assign hue names for saturated colors
 ...
 */
export function suggestComplementaryColors(currentColors: PaletteColor[], count: number = 3): PaletteColor[] {
  ...
  currentColors.forEach((color, index) => {
    ...
    if (!existingHexes.has(complementHex.toLowerCase()) && suggestions.length < count) {
      suggestions.push({
