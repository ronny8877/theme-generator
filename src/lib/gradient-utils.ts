...
export function addGradientStop(
	...
	return {
		...config,
		stops: [...config.stops, newStop].sort(
...
export function analyzeGradientContrast(config: GradientConfig): {
	...
	for (let i = 0; i < stops.length - 1; i++) {
		...
		const stepCount = Math.ceil(
			(nextStop.position - currentStop.position) / 5
		) // Every 5%
		...
		for (let j = 0; j <= stepCount; j++) {
			...
			const position =
				...
				(nextStop.position - currentStop.position) * t
			const color = interpolator(t)
...
function generateSVGGradient(config: GradientConfig): string {
	...
	if (config.type === 'linear') {
		const angle =
			typeof config.direction === 'number' ? config.direction : 0
