		...
		<div className={`space-y-4 ${className}`}>
			...
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-sm text-base-content/70">
						Simulating {simulationInfo.name} (
			...
			<div className="grid gap-4">
				{colors.map((originalColor, index) => {
					const simulatedColor = simulateColor(
						originalColor,
						simulationType
					)
					const isDifferent =
						originalColor.toLowerCase() !==
						simulatedColor.toLowerCase()

					return (
						<div
							key={index}
							className="p-4 bg-base-100 rounded-lg border"
						>
			...
			<div className="p-4 bg-base-200 rounded-lg border">
				<div className="text-sm">
					...
					<p className="text-base-content/70">
						...
						out of {colors.length} colors appear different to people
						with {simulationInfo.name}.
					</p>
					{simulationInfo.severity === 'severe' && (
						<p className="text-warning text-xs mt-2">
							⚠️ This is a severe condition that significantly
