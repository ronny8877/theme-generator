...
export function CommonToolbar({ className = "" }: ToolbarProps) {
  ...
  return (
    <div
      className={`navbar bg-base-100 shadow-lg border-b border-base-200 ${className}`}
    >
      ...
      <div className="navbar-center hidden lg:flex">
        <ul className="menu menu-horizontal px-1 gap-2">
          {tools.slice(1, -1).map((tool) => {
            // Skip Home and FAQ for main nav
            const Icon = tool.icon;
            return (
              <li key={tool.href}>
                <Link
...
export function CompactToolbar({ className = "" }: ToolbarProps) {
  ...
  return (
    <div
      className={`flex gap-2 p-4 bg-base-100 border-b border-base-200 ${className}`}
    >
      {tools.map((tool) => {
        const Icon = tool.icon;
        return (
