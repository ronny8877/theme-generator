...
import { useAppActions, useFontActions, useTemplateActions } from "@/store/hooks";
...
const Showcase: React.FC<ShowcaseProps> = ({ className = "", onThemeChange }) => {
  ...
  const { setActiveTemplateById, applyThemeAndResetBaseline } = useTemplateActions();
  const { resetBaselineToCurrent, setActiveEditorTab, setActivePanel } = useAppActions();
  ...
  return (
    <section className={`mt-10 ${className}`}>
      <div className="flex flex-col gap-5 items-center">
        <TemplatePills activeId={templateId} onSelect={setTemplateId} className="bg-base-200 py-5 px-3 rounded-4xl" />

        <div className="relative w-full rounded-3xl bg-base-200 p-4">
          {/* Floating controls on the right, stacked vertically */}
          <div className="hidden lg:flex flex-col gap-2 absolute right-6 top-6 z-10">
            <DaisyThemeSelect label="Theme" value={theme} onSelect={onSelectTheme} />
            <FontSelect label="Heading" value={headingFont} onSelect={setHeadingFont} />
            <FontSelect label="Body" value={bodyFont} onSelect={setBodyFont} />
          </div>

          {/* Smaller preview frame */}
          <div className="max-w-[1024px] mx-auto">
            <PreviewFrame theme={theme} className="rounded-3xl overflow-hidden" />
          </div>
        </div>
