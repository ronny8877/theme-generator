...
const GENERATION_MODES: { value: PaletteGenerationMode; label: string; description: string }[] = [
  { value: 'random', label: 'Random', description: 'Completely random colors' },
  { value: 'monochromatic', label: 'Monochromatic', description: 'Same hue, different lightness' },
  ...
  { value: 'tetradic', label: 'Tetradic', description: 'Four evenly spaced colors' },
...
export function PaletteTool() {
  ...
  const [generationMode, setGenerationMode] = useState<PaletteGenerationMode>('random');
  ...
  const [savedPalettes, setSavedPalettes] = useState<SavedPaletteData>({ palettes: [], tags: [] });
  const [showSavedPalettes, setShowSavedPalettes] = useState(false);
  const [searchQuery, setSearchQuery] = useState('');
  const [selectedTags, setSelectedTags] = useState<string[]>([]);
  const [palettePreviewMode, setPalettePreviewMode] = useState<'website' | 'card' | 'dashboard' | 'mobile'>('website');
  const [showPreview, setShowPreview] = useState(false);
  const [suggestions, setSuggestions] = useState<PaletteColor[]>([]);
  const [currentPaletteName, setCurrentPaletteName] = useState('Generated Palette');
  ...
  const generateNewPalette = React.useCallback(() => {
    ...
    const finalColors = newColors.map((newColor, index) => {
      const existingLocked = preservedColors.find((_, lockedIndex) => lockedIndex === index);
  ...
  const filteredPalettes = React.useMemo(() => {
    ...
    return searchPalettes(searchQuery, selectedTags.length > 0 ? selectedTags : undefined);
  ...
  const exportPaletteAs = (format: 'css' | 'json' | 'tailwind' | 'svg' | 'png') => {
    const palette: ColorPalette = {
      id: 'current-palette',
      name: currentPaletteName,
      colors: currentPalette,
    ...
    <div className="space-y-8">
        ...
        <div className="card-body">
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-3">
                <label className="label">
                  <span className="label-text font-semibold">Generation Mode</span>
                </label>
                <select
            ...
            <div className="flex items-center gap-6">
              ...
              <Button onClick={generateNewPalette} className="btn-primary flex-1 max-w-xs">
        ...
        <div className="card-body">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex gap-3">
              <Button size="sm" variant="outline" onClick={improvePalette} className="btn-sm">
                <Sparkles className="h-4 w-4 mr-2" />
                Improve
              </Button>
              <Button size="sm" variant="outline" onClick={handleCopyPalette} className="btn-sm">
          ...
          <div className="card-body">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-6">
              <div className="stat bg-base-200 rounded-2xl shadow-md">
                <div className="stat-value text-3xl">{analysis.averageContrast.toFixed(1)}</div>
                <div className="stat-title">Avg Contrast</div>
                <div className="stat-desc">Higher is better</div>
              </div>
              
              <div className="stat bg-base-200 rounded-2xl shadow-md">
                <div className="stat-value text-3xl">{(analysis.diversity * 100).toFixed(0)}%</div>
                <div className="stat-title">Diversity</div>
                <div className="stat-desc">Color variation</div>
              </div>
              
              <div className="stat bg-base-200 rounded-2xl shadow-md">
                <div className="stat-value text-2xl capitalize">{analysis.harmony}</div>
              ...
              <div className="stat bg-base-200 rounded-2xl shadow-md">
                <div className="stat-value text-3xl">
                  {analysis.contrastPairs.filter(p => p.accessible).length}/{analysis.contrastPairs.length}
                </div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid gap-3">
                  {analysis.contrastPairs.map((pair, index) => (
                    <div key={index} className="flex items-center justify-between p-4 rounded-2xl border-2 border-base-300 bg-base-50">
                      ...
                      <div className="flex items-center gap-3">
                        <span className="text-lg font-mono font-semibold">{pair.ratio.toFixed(1)}:1</span>
      ...
      <div className="card bg-base-100 shadow-lg">
        <div className="card-body">
            ...
            <div className="flex gap-2">
              {[
                ...
                <Button
                  key={mode.value}
                  size="sm"
                  variant={palettePreviewMode === mode.value ? "default" : "outline"}
                  onClick={() => setPalettePreviewMode(mode.value as 'website' | 'card' | 'dashboard' | 'mobile')}
                  className="btn-sm"
                >
          ...
          <PalettePreview colors={currentPalette} mockupType={palettePreviewMode} />
        </div>
      </div>
              ...
              {suggestions.map((suggestion) => (
                <div key={suggestion.id} className="space-y-2">
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-xs text-base-content/60">{suggestion.hex}</p>
                  </div>
                </div>
              ))}
            </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {currentPaletteTags.map((tag) => (
                    <div key={tag} className="badge badge-primary badge-lg gap-2">
          ...
          {showSavedPalettes && (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="label">
                    <span className="label-text font-semibold">Search Palettes</span>
                  </label>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredPalettes.map((palette) => (
                  <div key={palette.id} className="card card-compact bg-base-200 shadow-md">
                    <div className="card-body">
