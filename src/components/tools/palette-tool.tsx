...
export function PaletteTool() {
	...
	const handleImageUpload = async (
		...
		try {
			...
			const newSuggestions = suggestComplementaryColors(
				extractedColors,
				5
			)
			setSuggestions(newSuggestions)
			...
			toast.success(
				`Extracted ${extractedColors.length} colors from image`
			)
		} catch (error) {
							...
							<div className="space-y-3">
								...
								<select
									...
									onChange={(e) =>
										setGenerationMode(
											e.target
												.value as PaletteGenerationMode
										)
									}
								...
								<p className="text-sm text-base-content/70">
									{
										GENERATION_MODES.find(
											(m) => m.value === generationMode
										)?.description
									}
								</p>
							</div>
										...
										(pair, index) => (
											...
											>
												...
												<div className="flex items-center gap-3">
													...
													<div
														className={`badge ${pair.rating.class} badge-lg`}
													>
														{pair.ratio >= 3 && (
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
							<div>
								...
								<input
									...
									onChange={(e) =>
										setCurrentPaletteName(e.target.value)
									}
